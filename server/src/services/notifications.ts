...
export const createNotification = async (c: ctx, email: string, body: INotification) => {
  return getSupabase(c).from('notifications').insert({
    email,
    notifications: [body]
  }).select();
};

export const updateNotifications = async (c: ctx, email: string, body: INotification[]) => {
