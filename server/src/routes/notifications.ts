...
import { createNotification, getNotifications, updateNotifications } from '@server/services/notifications';
...
notificationsApp.get('/:email', async (c) => {
  ...
  const { data, error } = await getNotifications(c, c.req.param('email') as string);  

  if (error) return c.json({ error }, 400);

  if (data && data.length > 0) {
    return c.json({ data: data[0].notifications }, 200);
  }
...
notificationsApp.put('/:email', async (c) => {
  ...
  const { data: existingData, error: fetchError } = await getNotifications(c, email);
